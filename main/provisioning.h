/*******************************************************************************
  ...
  * @version
  * @date
  * @brief      Lets a technician set the device's WiFi credentials and
  *             server address over the same USB/UART console used for log
  *             output and `idf.py monitor`, instead of baking them into
  *             `sdkconfig` at compile time. Values are persisted to NVS and
  *             take priority over the `idf.py menuconfig` (Kconfig)
 ...
 *         protocol §1.2), applying and persisting each one it understands to
 *         NVS as it arrives and printing a one-line JSON ack/error back over
 *         the same UART. The window closes early once `idle_timeout_ms`
 *         passes with no byte received, so an unattended device does not
 *         wait the full `max_total_ms` on every power-on.
